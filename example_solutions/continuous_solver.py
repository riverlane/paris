...
def continuous_solver(training_data):
    ...
    for train_vector, train_label in training_data:
        execution_result = execute(best_circ, simulator, backend_options={"initial_statevector": train_vector}).result()
        predicted_label = compute_parity_exp_value(execution_result.get_statevector(best_circ))
        training_error += abs(train_label - predicted_label)

    return infererance_retval(
            infer_fun = infer,
            infer_circ = best_circ,
            training_error = training_error,
            discription = "Circuit generated by continuous search in parameter space"
        )
